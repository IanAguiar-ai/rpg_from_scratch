...
class BaseEntity:
    def __init__(self, name:str, hp:float, speed:float, aceleration:float, size:float,
                 ...
                 frame:str = None, pos:list[float] = [5, 5], colision:bool = False) -> None:
...
#######################################################################################
class BaseAtk:
    def __init__(self, damage:int, speed:float, pos:list[float], pos_final:list[float], poison:int = None, size:float = 0.01, life_span:int = 10, 
...
def projectile_arrow(owner, target, damage:int = 20, speed:float = 0.4, size = 0.03, life_span = 25, player = None):
    return BaseAtk(damage = damage, speed = speed, pos = owner, pos_final = target,
                   size = size, life_span = life_span)

def projectile_around(owner, target, damage:int = 25, speed:float = 0.04, size = 0.04, life_span = 40, player = None):
    return [BaseAtk(damage = damage, speed = speed, pos = [target[0] + x, target[1] + y], pos_final = [target[0], target[1]],
                    size = size, life_span = life_span) for x, y in ((-1, -1), (-1, 1), (1, -1), (1, 1))]
